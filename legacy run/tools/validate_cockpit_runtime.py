...
def main():
    ...
    carla_root = Path(os.environ.get('CARLA_ROOT', str(get_carla_root())))
    ...
    with log_path.open('w', encoding='utf-8') as log:
        process = subprocess.Popen([
            sys.executable, '-u', str(root / 'app' / 'spawn_car.py'),
        ...
        try:
            ...
            report = dict(exit_code=code, duration_s=round(time.monotonic()-started,1),
                          ...
                          log=str(log_path))
            (root / 'logs' / (args.label + '.json')).write_text(json.dumps(report, indent=2), encoding="utf-8")

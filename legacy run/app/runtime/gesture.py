...
def policy_bias_from_payload(payload, cap=GESTURE_BIAS_CAP):
    ...
    active = isinstance(payload, dict) and payload.get("tracking") is True and any(
...
class GestureBiasReceiver:
    ...
    def __init__(self, host="127.0.0.1", port=8765, stale_after_s=0.75):
        if not ipaddress.ip_address(host).is_loopback:
            raise ValueError("Gesture input must bind to a loopback address")
        self.host = host
        self.port = int(port)
        self.stale_after_s = float(stale_after_s)
        if not math.isfinite(self.stale_after_s) or self.stale_after_s <= 0:
            raise ValueError("Gesture expiry must be finite and positive")

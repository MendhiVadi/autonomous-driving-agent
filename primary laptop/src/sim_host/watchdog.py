...
class CommandWatchdog:
    ...
    def suspend(self):
        """Pause the lease during a serialized synchronous reset/physics step.

        The caller must ensure physics cannot free-run while suspended.
        Re-arm with command_received() when the next command is invited.
        """
        with self._lock:
            self._armed = False

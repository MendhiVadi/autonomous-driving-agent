...
from evaluation_dashboard import summarize, write_dashboard
from neural_drive_agent import (MLP, apply_stall_recovery,
                                load_deployable_policy, make_observation)
from safety_supervisor import SafetySupervisor
from control_policy import VehicleCommand, steering_angle_from_lane_error
from device_watchdog import CommandWatchdog
from map_features import _choose_continuation
from policy_contract import POLICY_CONTRACT_VERSION, normalize_features
from route_planner import _load_agents
from manual_transmission import (EngineRpmEstimator, ManualTransmissionConfig,
                                 automatic_gear_for_speed, estimate_engine_rpm,
                                 gear_redline_speed_kmh,
                                 recommended_shift)
from gesture_policy_bias import GESTURE_BIAS_CAP, policy_bias_from_payload
from health_guard import HealthEvent, evaluate_health
...
CARLA_ROOT = Path(r"C:\Users\medha\Documents\CARLA\CARLA_0.9.16")


def load_spawn_module():
    spec = importlib.util.spec_from_file_location("cockpit_spawn", CARLA_ROOT / "spawn_car.py")
...
class CoreLogicTests(unittest.TestCase):
    ...
    def test_supervisor_brakes_on_real_command_age(self):
        command, reasons = SafetySupervisor().apply(
            VehicleCommand(accelerator=0.5), {"speed_kmh": 10.0, "command_age_s": 1.0}
        )
    ...
    def test_policy_rejects_wrong_layer_dimensions(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps({"sizes": [2, 1], "weights": [[[1.0]]], "biases": [[0.0]]}))
...
class SupervisorCommandLineTests(unittest.TestCase):
    ...
    def test_default_renderer_is_dx12(self):
        self.assertIn("-dx12", self._carla_argv([]))

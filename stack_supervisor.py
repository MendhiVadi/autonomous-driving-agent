...
from health_guard import check_system_health


DEFAULT_CARLA_ROOT = Path(r"C:\Users\medha\Documents\CARLA\CARLA_0.9.16")
...
def carla_rpc_probe(host: str, port: int, timeout_s: float = 3.0) -> bool:
    ...
    try:
        ...
        # A frozen asynchronous world must not pass readiness either.
...
def terminate_tree(pid: int, timeout_s: float = 10.0) -> bool:
    """Kill a process and everything it spawned. Returns True if taskkill ran."""
...
class Supervisor:
    ...
    def sweep_orphans(self) -> None:
        """Clear leftovers from an earlier run before starting our own.

        Without this, a CARLA server orphaned by a shim-only kill keeps port
        2000 while we start a second one - two renderers on one iGPU.
        """
        mine = {os.getpid()}
        orphans = [pid for pid in find_orphans() if pid not in mine]
        if not orphans:
            return
        self.log(f"clearing {len(orphans)} leftover stack process(es) from a previous run")
        for pid in orphans:
            terminate_tree(pid)
        time.sleep(2.0)
...
def build_components(args) -> list:
    ...
    neural = Component(
        name="neural",
        argv=[str(carla_python), str(app_root / "neural_drive_agent.py"),
        ...
        probe=None,  # it logs its own health; a live PID plus CARLA's probe is enough
        depends_on=("carla",),
        ready_grace_s=60.0,
    )
    return [carla, neural]
...
def parse_args(argv=None):
    ...
    parser.add_argument("--render-api", default="dx12",
    ...
    parser.add_argument("--no-orphan-sweep", action="store_true",
                        help="Leave any stack processes from a previous run alone")
...
def main(argv=None) -> int:
    ...
    supervisor = Supervisor(
        ...
        sweep_on_start=not args.no_orphan_sweep,
